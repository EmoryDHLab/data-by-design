import {chapterMeta} from "./app/data/chapterMeta";
import type { Config } from 'tailwindcss'

export default {
  content: ["./app/**/*.{js,ts,jsx,tsx}"],
  important: true,
  media: false, // or 'media' or 'class'
  theme: {
    screens: {
      sm: "640px",
      // => @media (min-width: 640px) { ... }

      md: "768px",
      // => @media (min-width: 768px) { ... }

      lg: "1024px",
      // => @media (min-width: 1024px) { ... }

      xl: "1280px",
      // => @media (min-width: 1280px) { ... }

      "2xl": "1536px",
      // => @media (min-width: 1536px) { ... }
      "3xl": "1896px",
      // => @media (min-width: 1536px) { ... }
    },
    extend: {
      fontFamily: {
        dubois: ["VTC Du Bois, serif", { fontFeatureSettings: ' "ss06"' }],
        duboisWide: ["VTC Du Bois Wide", "serif"],
        duboisLightWide: ["VTC Du Bois Light Wide", "serif"],
        duboisNarrow: ["VTC Du Bois Narrow", "serif"],
        duboisLightNarrow: ["VTC Du Bois Light Narrow", "serif"],
        neueMontreal: ["PPNeueMontreal"],
        neueMontrealBold: ["PPNeueMontreal Bold"],
        neueMontrealLight: ["PPNeueMontreal Light"],
        sans: ["PPNeueMontreal", "sans-serif"],
        icons: ["DxD Icons"],
      },
      spacing: {
        18: "4.5rem",
      },
      gridTemplateColumns: {
        14: "repeat(14, minmax(0, 1fr))",
      },
      backgroundImage: {
        description: `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('${chapterMeta.description.bgImage}')`,
        dubois: `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('${chapterMeta.dubois.bgImage}')`,
        intro: `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('${chapterMeta.intro.bgImage}')`,
        labor: `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('${chapterMeta.labor.bgImage}')`,
        peabody: `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('${chapterMeta.peabody.bgImage}')`,
        playfair: `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('${chapterMeta.playfair.bgImage}')`,
        shanawdithit: `linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url('${chapterMeta.shanawdithit.bgImage}')`,
      },
      colors: {
        black: "#1C1817",
        offblack: "#1C1817",
        offwhite: "#FDF9F6",
        nightingale_blue: "#3B6FE0",
        peabodyOrange: "rgb(219, 136, 42)",
        descriptionSecondary: {
          DEFAULT: "#E0DCF2",
          translucent: "#E0DCF2CC",
        },
        descriptionPrimary: {
          DEFAULT: "#8C20E1",
          translucent: "#8c20e1cc",
        },
        duboisTextColor: "white",
        duboisSecondary: {
          DEFAULT: "#FFD3D3",
          translucent: "#FFD3D3CC",
        },
        duboisPrimary: {
          DEFAULT: "#D92944",
          translucent: "#D92944CC",
        },
        playfairSecondary: {
          DEFAULT: "#D1E4FF",
          translucent: "#D1E4FFCC",
        },
        playfairPrimary: {
          DEFAULT: "#3B4BE0",
          translucent: "#3B4BE0CC",
        },
        peabodySecondary: {
          DEFAULT: "#DBF5E9",
          translucent: "#DBF5E9CC",
        },
        peabodyPrimary: {
          DEFAULT: "#9AE4C1",
          translucent: "#9AE4C1CC",
        },
        laborPrimary: {
          DEFAULT: "#BB9AEC",
        },
        labourSecondary: {
          DEFAULT: "#DFD2DE",
        },
        peabodyPrimaryHalfOpacity: "#9ae4c180",
        shanawdithitPrimary: "#fb9318",
        shanawdithitSecondary: "#fac88d",
        shanawdithitAccent:"#FEFFC9",
        peabodyChartBackground: "#d9bb9f",
        willardSecondary: "#FEFFC9",
        willardPrimary: "#FEC313",
        England: "rgb(119,43,21)",
        France: "rgb(60,100,100)",
        Americas: "rgb(222,145,49)",
        Holland: "rgb(68,108,73)",
        Sweden: "rgb(217,182,17)",
        Spain: "rgb(209, 42, 5)",
        Native: "url(#diagonalHatch)",
        Mexico: "rgb(209, 42, 5)",
        playfairChart: "#F3ECCB",
      },
      inset: {
        screen: "100vh",
        "-screen": "-100vh",
      },
      fontSize: {
        frontTitle: "8rem",
      },
      lineHeight: {
        veryTight: "0.8",
      },
    },
  },
  variants: {
    extend: {},
  },
  safelist: [
    "bg-shanawdithitAccent",
    "shanawdithitAccent",
    "bg-descriptionPrimary",
    "bg-laborPrimary",
    "border-descriptionPrimary",
    "border-l-descriptionPrimary",
    "border-l-shanawdithitPrimary",
    "border-l-peabodyPrimary",
    "border-l-playfairPrimary",
    "border-l-duboisPrimary",
    "stroke-descriptionPrimary",
    "stroke-shanawdithitPrimary",
    "stroke-peabodyPrimary",
    "stroke-playfairPrimary",
    "stroke-duboisPrimary",
    "stroke-laborPrimary",
    "fill-descriptionPrimary",
    "fill-shanawdithitPrimary",
    "fill-peabodyPrimary",
    "fill-peabodyOrange",
    "fill-playfairPrimary",
    "fill-duboisPrimary",
    "fill-descriptionSecondary",
    "fill-descriptionPrimary/50",
    "fill-shanawdithitPrimary/50",
    "fill-peabodyPrimary/50",
    "fill-playfairPrimary/50",
    "fill-duboisPrimary/50",
    "fill-descriptionSecondary/50",
    "fill-offwhite",
    "fill-offblack",
    "hover:bg-descriptionSecondary",
    "hover:bg-shanawdithitSecondary",
    "hover:bg-peabodySecondary",
    "hover:bg-playfairSecondary",
    "hover:bg-duboisSecondary",
    "hover:bg-descriptionPrimary",
    "hover:bg-shanawdithitPrimary",
    "hover:bg-peabodyPrimary",
    "hover:bg-playfairPrimary",
    "hover:bg-duboisPrimary",
    "hover:fill-descriptionPrimary",
    "hover:fill-shanawdithitPrimary",
    "hover:fill-peabodyPrimary",
    "hover:fill-playfairPrimary",
    "hover:fill-duboisPrimary",
    "hover:stroke-descriptionPrimary",
    "hover:stroke-shanawdithitPrimary",
    "hover:stroke-peabodyPrimary",
    "hover:stroke-playfairPrimary",
    "hover:stroke-duboisPrimary",
    "text-descriptionPrimary",
    "bg-descriptionSecondary",
    "bg-descriptionPrimary-translucent",
    "bg-descriptionSecondary-translucent",
    "bg-duboisPrimary",
    "bg-duboisSecondary",
    "text-duboisPrimary",
    "text-playfairPrimary",
    "hover:text-duboisPrimary",
    "hover:text-playfairPrimary",
    "focus:text-duboisPrimary",
    "focus:text-playfairPrimary",
    "hover:text-peabodyPrimary",
    "focus:text-peabodyPrimary",
    "hover:text-laborPrimary",
    "focus:text-laborPrimary",
    "hover:text-shanawdithitPrimary",
    "focus:text-shanawdithitPrimary",
    "hover:text-descriptionPrimary",
    "focus:text-descriptionPrimary",
    "bg-playfairPrimary",
    "bg-playfairPrimary-translucent",
    "bg-playfairSecondary",
    "bg-playfairSecondary-translucent",
    "bg-duboisPrimary-translucent",
    "bg-playfairChart",
    "bg-shanawdithitPrimary",
    "bg-shanawdithitSecondary",
    "text-peabodyPrimary",
    "text-peabodyOrange",
    "bg-peabodyPrimary",
    "bg-peabodySecondary",
    "bg-peabodyPrimary-translucent",
    "transition-opacity",
    "opacity-0",
    "opacity-10",
    "opacity-25",
    "opacity-100",
    "duration-700",
    "text-offblack",
    "font-extrabold",
    "bg-England",
    "bg-France",
    "bg-Americas",
    "bg-Holland",
    "bg-Sweden",
    "bg-Spain",
    "bg-Native",
    "bg-Mexico",
    "font-icons",
    "outline-4",
    "outline-red-500",
    "outline-solid",
    "scale-0",
    "scale-100",
    "stroke-white",
    "fill-white",
    "fill-black",
    "fill-yellow-500",
    "stroke-yellow-500",
    "fill-gray-300",
    "fill-gray-50",
    "text-green-400",
    "text-red-600",
    "text-red-800",
    "text-lime-500",
    "top-12",
    "top-24",
    "top-32",
    "md:scale-0",
    "h-0",
    "h-24",
    "h-48",
    "h-2/5",
    "h-20",
    "h-[80vh]",
    "h-[125vh]",
    "grid-rows-1",
    "drop-shadow-2xl",
    "underline-offset-8",
    "translate-x-[2px]",
    "md:-mx-64",
    "border-opacity-0",
    "border-opacity-70",
    "opacity-20",
    "md:mt-28",
    ...Object.keys(chapterMeta).map((chapter) => `bg-${chapter}`),
    ...Object.keys(chapterMeta).map((chapter) => `hover:bg-${chapter}`),
    ...Object.keys(chapterMeta).map((chapter) => `selection:bg-${chapter}Primary`),
    ...Object.keys(chapterMeta).map((chapter) => `selection:bg-${chapter}Secondary`),
    ...Object.keys(chapterMeta).map((chapter) => `outline-${chapter}Primary`),
    ...Object.keys(chapterMeta).map((chapter) => `outline-${chapter}Secondary`),
  ],
} satisfies Config;
