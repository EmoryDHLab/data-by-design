...
import type { PeabodyActor, QuizStepCount } from "~/types/process";
...
export default function QuizSelectActors() {
  const { currentStep, currentStepCount, setCurrentStepCount, setFeedback } =
  ...
  const selectActor = (actor: PeabodyActor) => {
    ...
    if (currentStep?.stepEvent?.actors.includes(actor)) {
      if (solvedActors.length < 1) {
        ...
        setFeedback({
          message:
            "Well done. Now, click the year the when Powhatan attacked Virginia.",
          correct: true,
        });
      }
      ...
      setFeedback({
        message: `No, ${actor} was not involved. Click the remaining ${
          2 - solvedActors.length
        } involved.`,
        correct: false,
      });
  ...
  return (
    <g className={`transition-all duration-1000`}>
      {actors.map((actor, index) => {
        if (
          (currentStepCount > 1 &&
            correctActors.includes(actor as PeabodyActor)) ||
          currentStepCount == 1
        ) {
