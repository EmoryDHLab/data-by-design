...
const SiteNav = () => {
  ...
  return (
    <>
      <nav
        className={`flex flex-col md:flex-row bg-black/95 text-offwhite font-power fixed top-0 bottom-0 w-screen h-screen transition-transform duration-500 z-50 overflow-y-hidden overflow-x-hidden md:pt-24 ${
          show ? "translate-y-0" : "-translate-y-full"
        }`}
      >
        <ol className="w-full text-2xl ps-8">
          <li className="mb-4">
            <Link to="/">DxD</Link>
          </li>
          <li className="">
              ...
              >
                <Link to={`/chapters/${chapter}`} className="">
                  {index < 2 ? (
                    ...
                    <div className="text-xs pt-2 ps-2">Chapter {index - 1}</div>
                  )}
                  <div className="capitalize flex flex-row md:text-xl pt-2 ps-2">
