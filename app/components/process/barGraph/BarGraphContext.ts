...
import type { ActivePeabodyEvent, PeabodyEvent } from "~/types/process";
...
const BarGraphContext = createContext<IBarGraphContext>({
  ...
  setCurrentCentury: (_: SetStateAction<PeabodyCentury | number>) => {
    console.error(
      "setCurrentCentury not implemented. Did you pass it to context?"
    );
  },
  currentCenturyEvents: [],
  setCurrentCenturyEvents: (_: SetStateAction<Array<PeabodyEvent>>) => {
    console.error(
      "setCurrentCenturyEvents not implemented. Did you pass it to context?"
    );
  },
  activeEvent: undefined,
  setActiveEvent: (_: SetStateAction<ActivePeabodyEvent>) => {
    console.error(
      "setActiveEvent not implemented. Did you pass it to context?"
    );
  },
  centuries: [],
});
