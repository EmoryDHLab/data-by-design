...
import eventsData from "~/data/process/1600sEvents.json";
...
export default function Tutorial() {
  ...
  const [focusShapeSize, setFocusShapeSize] = useState<Focus>({
    x: 0,
    y: 0,
    w: 100,
    h: 100,
          ...
          <svg viewBox="0 0 99 99" className="mt-8 mx-8">
            <g mask="url(#tutorial-mask)">
              ...
              {[...numberRange(1601, 1700)].map((year, index) => {
                const yearEvents = eventsData.events.filter(
                    ...
                    {yearEvents && (
                      <>
                        {[...numberRange(0, 8)].map((eventIndex) => {
                          const yearEvent = yearEvents?.find(
                            (event) =>
                              (event?.squares as Array<number>).includes(
                                eventIndex + 1
                              ) || event?.squares === "full"
                          );
                          return (
