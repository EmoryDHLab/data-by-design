...
const StudentCharts = ({ figure }: { figure: TFigure }) => {
  ...
  return (
    <section
      className="w-full bg-black grid grid-cols-4 justify-items-center"
      id="student-charts"
    >
      <nav className="">
        <ul className="my-8">
          <li className="my-4">
            <button
              className={`${
                currentChart === "original" ? "bg-duboisSecondary" : "bg-white"
      ...
      <div className="col-span-3 m-6">
        <div className="bg-offwhite text-black py-2 px-10">
          ...
          {currentChart === "one" && <StudentChartOne />}
          {currentChart === "two" && <StudentChartTwo />}
